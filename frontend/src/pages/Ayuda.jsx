  ...
  return (
    <>
      <style>{`
        ...
        .ayuda-sub { font-size: 0.78rem; color: rgba(255,255,255,0.3); text-transform: uppercase; letter-spacing: 0.08em; margin: 0 0 1.5rem; font-weight: 300; }
        .ayuda-tabs { display: flex; gap: 0; overflow-x: auto; }
        .ayuda-tab {
      ...
      <div className="ayuda-root">
            ...
            {SECCIONES.map(s => {
              ...
              return (
                <button
                  ...
                  className={`ayuda-tab ${seccionActiva === s.id ? 'activo' : ''}`}
                  style={seccionActiva === s.id ? { color: s.color, borderBottomColor: s.color } : {}}
                >
                  <Icono size={14} />
                  {s.titulo}
                </button>
        ...
        {SECCIONES.filter(s => s.id === seccionActiva).map(seccion => (
          <div key={seccion.id} className="ayuda-body">
            <h2 className="ayuda-seccion-titulo">{seccion.titulo}</h2>
            <p className="ayuda-seccion-sub">{seccion.subtitulo}</p>
            <div className="ayuda-items">
